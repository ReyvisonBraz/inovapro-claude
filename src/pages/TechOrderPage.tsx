...
import { ArrowLeft, Edit, Smartphone, Calendar, Clock, User, CheckCircle2, AlertTriangle } from 'lucide-react';
...
export const TechOrderPage: React.FC = () => {
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    try {
      ...
      setSuccessMsg(`Status atualizado para "${newStatus}"`);
  ...
  const isPastDeadline = order.analysisPrediction
  ...
  return (
    <div className="min-h-screen bg-bg-dark text-slate-100">
      {/* Header */}
      <div className="sticky top-0 z-10 bg-bg-dark/90 backdrop-blur-md border-b border-white/10 px-4 py-3 flex items-center justify-between">
        <button
          onClick={() => navigate(-1)}
        ...
        <button
          ...
          className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-primary text-white text-xs font-bold hover:bg-primary/90 transition-all active:scale-95"
        >
          <Edit size={14} />
          Editar
        </button>
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-5">
        ...
        <div className="glass-card p-4">
          <div className="flex items-center justify-between mb-4">
            <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Status Atual</p>
            {currentStatus && (
              <span
            ...
            {statuses.map(s => (
              <button
                ...
                style={{
                  ...
                  ...(order.status === s.name ? { ringColor: s.color } : {}),
                }}
              >
                <span
