...
import {
  ...
  CheckCircle2, AlertTriangle, Image, ClipboardList, Phone, Wrench,
} from 'lucide-react';
import api from '../lib/api';
import { useAppStore } from '../store/useAppStore';
import { cn } from '../lib/utils';
import { useToast } from '../components/ui/Toast';
import { parsePhotos } from './tech/parsePhotos';
import { Lightbox } from './tech/Lightbox';
import { QuickSaveField } from './tech/QuickSaveField';
...
export const TechOrderPage: React.FC = () => {
  ...
  const isPastDeadline = order.analysisPrediction && new Date(order.analysisPrediction) < new Date() && order.status !== 'Concluído' && order.status !== 'Entregue';
  const photos = parsePhotos(order.arrivalPhotoUrls) || parsePhotos(order.arrivalPhotoBase64);

  return (
    <>
      {lightbox && (
        <Lightbox photos={lightbox.photos} index={lightbox.index} onClose={() => setLightbox(null)} onChange={i => setLightbox(prev => prev ? { ...prev, index: i } : null)} />
      )}
      ...
      <div className="min-h-screen bg-bg-dark text-slate-100">
        <div className="sticky top-0 z-10 bg-bg-dark/90 backdrop-blur-md border-b border-white/10 px-4 py-3 flex items-center justify-between gap-3">
          ...
          <span className="text-xs font-black text-primary bg-primary/10 px-3 py-1 rounded-full border border-primary/20 shrink-0">{osNumber}</span>
          <button onClick={handleOpenEdit} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-primary text-white text-xs font-bold hover:bg-primary/90 transition-all active:scale-95 shrink-0">
            <Edit size={14} /> Editar
          </button>
        </div>
        ...
        <div className="max-w-lg mx-auto px-4 py-6 space-y-4">
          ...
          <div className="glass-card p-4 space-y-3">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 rounded-xl bg-primary/10 flex items-center justify-center text-primary shrink-0"><User size={18} /></div>
              <div className="min-w-0 flex-1">
                <p className="text-lg font-black text-white truncate">{order.firstName} {order.lastName}</p>
                <div className="flex flex-wrap items-center gap-2 mt-0.5">
                  ...
                  {order.phone && (
                    <a href={`tel:${order.phone}`} className="inline-flex items-center gap-1 text-[10px] font-bold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded-full hover:bg-emerald-500/20 transition-all">
              ...
              {order.entryDate && (
                <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-blue-500/5 border border-blue-500/10">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-bold text-blue-300">{order.entryDate.split('-').reverse().join('/')}</p>
                  </div>
                </div>
              )}
              {order.analysisPrediction && (
                <div className={cn("flex items-center gap-2 px-3 py-2 rounded-lg border", isPastDeadline ? "bg-rose-500/5 border-rose-500/10" : "bg-slate-500/5 border-slate-500/10")}>
          ...
          {photos.length > 0 && (
            <div className="glass-card p-4 space-y-3">
              <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-1.5">
                <Image size={10} className="text-primary/60" /> Fotos de Entrada ({photos.length})
              </p>
              <div className="grid grid-cols-3 gap-2">
                {photos.map((src, idx) => (
                  <button key={idx} onClick={() => setLightbox({ photos, index: idx })} className="aspect-square rounded-xl overflow-hidden border border-white/10 bg-white/5 relative group hover:border-primary/40 transition-all active:scale-95">
          ...
          {order.reportedProblem && (
            <div className="glass-card p-4">
              <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2 flex items-center gap-1.5">
                <AlertTriangle size={10} className="text-primary/60" /> Problema Relatado
              </p>
          ...
          {/* Quick Analysis */}
          <QuickSaveField icon={<ClipboardList size={10} className="text-primary/60" />} label="Análise Técnica" placeholder="Descreva a análise técnica do equipamento..." value={order.technicalAnalysis || ''} onSave={v => handleSaveField('technicalAnalysis', v)} />

          {/* Quick Services */}
          <QuickSaveField icon={<Wrench size={10} className="text-primary/60" />} label="Serviços Realizados" placeholder="Liste os serviços realizados no equipamento..." value={order.servicesPerformed || ''} onSave={v => handleSaveField('servicesPerformed', v)} />
          ...
          <div className="glass-card p-4">
            <div className="flex items-center justify-between mb-4">
              ...
              {currentStatus && (
                <span className="text-xs font-bold px-3 py-1.5 rounded-full border flex items-center gap-1.5" style={{ backgroundColor: `${currentStatus.color}28`, color: currentStatus.color, borderColor: `${currentStatus.color}50` }}>
                  <span className="h-1.5 w-1.5 rounded-full bg-current animate-pulse" /> {order.status}
                </span>
              )}
            </div>

            {successMsg && (
              <div className={cn("flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold mb-3", successMsg.startsWith('Erro') ? "bg-rose-500/10 text-rose-400 border border-rose-500/20" : "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20")}>
            ...
            <div className="grid grid-cols-2 gap-2">
              {statuses.map(s => (
                <button key={s.id} onClick={() => handleStatusChange(s.name)} disabled={updatingStatus || order.status === s.name}
                  className={cn("flex items-center gap-2 px-3 py-3 rounded-xl border text-xs font-bold text-left transition-all active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed", order.status === s.name ? "ring-2 ring-offset-1 ring-offset-transparent" : "hover:brightness-125")}
          ...
          <button onClick={handleOpenEdit} className="w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl bg-primary text-white font-black text-sm hover:bg-primary/90 transition-all active:scale-[0.98] shadow-lg shadow-primary/20">
            <Edit size={16} /> Editar OS Completo
          </button>
