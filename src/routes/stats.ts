...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    // OS status breakdown for month
    const osStatusCount: Record<string, number> = {};
    for (const os of monthOS as any[]) {
      const s = os.status || 'Sem status';
      osStatusCount[s] = (osStatusCount[s] || 0) + 1;
    }

    // Top products/services from ALL service orders
    const serviceCount: Record<string, { qty: number; revenue: number }> = {};
    for (const os of allOS as any[]) {
      try {
        const services = JSON.parse(os.services || '[]');
      ...
      try {
        const parts = JSON.parse(os.partsUsed || '[]');
    ...
    res.json({
      ...
      monthOS,
      monthOSCount: (monthOS as any[]).length,
      osStatusCount,
      topProducts,
    });
  } catch (err) {
