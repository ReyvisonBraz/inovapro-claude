...
async function startServer() {
  ...
  app.delete("/api/customers/:id", (req, res) => {
    try {
      ...
      db.prepare("DELETE FROM service_orders WHERE customerId = ?").run(req.params.id);
      db.prepare("DELETE FROM client_payments WHERE customerId = ?").run(req.params.id);
      db.prepare("DELETE FROM customers WHERE id = ?").run(req.params.id);
      res.json({ success: true });
    } catch (error: any) {
      console.error("Error deleting customer:", error.message);
      res.status(400).json({ error: "Não foi possível excluir cliente. Verifique se há ordens de serviço ou pagamentos associados." });
    }
  });
